...
class LidarScanNode(Node):
	def __init__(self):
		...
		self.ranges = [float('inf')] * 500  # 500 statt 360
		self.received = set()
	...
	def publish_scan(self):
		while not incoming_text.empty():
			...
			try:
				...
				angle_index = int(angle_deg / 0.72) % 500 # 0,72*
				
				if angle_index < 0 or angle_index >= 500
				    self.get_logger().error(f"Ungültiger Index: {angle_index} für Winkel {angle_deg}°")
				    continue
