...
class kin_empfaenger(Node):
	def __init__(self):
		...
		self.de_pub = self.create_publisher(Image, '/kinect1/depth', 10)
		self.rgb_pub = self.create_publisher(Image, '/kinect1/rgb', 10)
		self.pc_pub = self.create_publisher(PointCloud2, '/kinect1/points', 10)
	...
	def start_depth_server(self):
		...
		server.listen(1)
		self.get_logger().info('TCP Depth Server läuft auf Port 5007')
	...
	def start_rgb_server(self):
		...
		server.listen(1)
		self.get_logger().info('TCP RGB Server läuft auf Port 5008')
	...
	def try_publish_pointcloud(self):
		...
		for v in range(self.height):
			for u in range(self.width):
				...
				rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, 0))[0]  # little-endian RGB
